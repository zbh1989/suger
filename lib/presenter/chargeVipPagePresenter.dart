...
class ChargeVipPagePresenter extends BasePresenter<ChargeVipPageState> {
  ...
  void createOrder(int type, int money, String orderNo) async {
    Future.wait([
      PreferenceUtils.instance.getString("payUrl").then(
            (val) => payUrl = val,
          ),
      PreferenceUtils.instance.getString("callUrl").then(
            (val) => callUrl = val,
          ),
      PreferenceUtils.instance.getString("token").then(
            (val) => token = val,
          ),
      PreferenceUtils.instance.getString("appId").then(
            (val) => appId = val,
          ),
      PreferenceUtils.instance.getString("userId").then(
            (val) => userId = val,
          ),
    ]).then((res) {
      if (payUrl != null &&
  ...
  void requestCreateOrder(String payType, int money, String orderNo) {
    ...
    try {
      ...
      requestFutureData<String>(Method.post, url: payUrl, queryParams: params,
          onSuccess: (data) {
        if (data != null) {
  ...
  void createCallbackOrder(String payType, int money, String orderNo) {
    try {
      ...
      requestFutureData<String>(Method.post,
          ...
          queryParams: params, onSuccess: (data) {
        if (data != null) {
          Map<String, dynamic> map = parseData(data);
          if (map['code'] == 200) {
