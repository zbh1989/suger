...
class ChargeGoldPagePresenter extends BasePresenter<ChargeGoldPageState>{
   ...
   *  extraGold: 赠送金币数
   */
  void createOrder(int type,int money,int gold,int extraGold,String orderNo) async {
    ...
    Future.wait([
      ...
      if(payUrl != null && callUrl != null && token != null && appId != null && userId != null){
        ...
        if(level < 0){
          view.showToast('未知充值金币等级，请通知开发排查...');
          return ;
        }

        /// 发送请求创建支付订单
        requestCreateOrder(payType,money,gold,extraGold,level,orderNo);
        /// 发送请求创建回调后台创建订单
        createCallbackOrder(payType,money,gold,extraGold,level,orderNo);
      }else{
   ...
   * 发送四方请求创建订单
   */
  void requestCreateOrder(String payType,int money,int gold,int extraGold,int level,String orderNo){
    ...
    try{
      ...
      params['version'] = '2.0';
      params['subject'] = '购物';
      Map body = {'money':money,'gold':gold,'extraGold':extraGold,'level':level};
   ...
   * 请求回调后台创建订单
   */
  void createCallbackOrder(String payType,int money,int gold,int extraGold,int level,String orderNo){
    try{
      ...
      Map body = {'money':money,'gold':gold,'extraGold':extraGold,'level':level,'bizType':2,'levelNo':level,};
