...
class DioUtils {
  ...
  DioUtils._internal() {
    _options = BaseOptions(
      baseUrl: Api.API_URL,
      connectTimeout: Duration(seconds:1000),//10000,

      ///响应流前后两次接收数据的间隔
      receiveTimeout: Duration(seconds:1000),//3000,
...
//    _dio.interceptors.add(CookieManager(CookieJar()));
  ...
  Future<BaseResponse<T>> _request<T>(String method, String url,
    ...
    try {
      BaseOptions options = BaseOptions(responseType: ResponseType.plain,connectTimeout: Duration(seconds:2000),);

      var response = await Dio(options).request(
          url,
      ...
      /*var response = await _dio.request(url,
          ...
          cancelToken: cancelToken);*/
      // Map<String, dynamic> _map =  http://206.238.76.51:10001/api/video/list
